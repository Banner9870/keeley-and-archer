...
| 5.9 | Implement feed ordering logic in `FeedPage.jsx`: four tiers — (1) Editor's Picks, (2) neighborhood-matched guides + articles interleaved by date, (3) category-matched guides + articles interleaved by date, (4) all remaining guides + articles interleaved by date. Within every tier, guides and articles are sorted together by recency — never separated into guide-first blocks. | M | Agent |
